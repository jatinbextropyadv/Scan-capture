 ...
 */
public class PdfKeywordValueFinder {
    ...
    private static final float OCR_MIN_CONFIDENCE = 20f;  // drop noise words (0-100)
    ...
    public PdfKeywordValueFinder(File pdfFile) throws Exception {
        this(pdfFile, new File("tessdata"));
    }

    /** @param tessdataDir directory containing eng.traineddata (only used for scanned PDFs) */
    public PdfKeywordValueFinder(File pdfFile, File tessdataDir) throws Exception {
    ...
    private static List<Token> extractOcrTokens(PDDocument document, File tessdataDir) throws Exception {
        ...
        tesseract.setPageSegMode(4);    // single column of variable-size text (sparse ticket)
    ...
    public static void main(String[] args) throws Exception 
    {
        ...
        if (System.getProperty("dump") != null) {
            for (Line l : finder.lines) System.out.printf("p%d y=%7.1f  %s%n", l.page, l.y, l.text());
        }
        for (int i = 1; i < args.length; i++) {
            ...
            System.out.printf("%-14s right=%-26s below=%-38s column=%s%n",
                    ...
                    finder.findValueInColumnBelow(kw));
        }
    }
}