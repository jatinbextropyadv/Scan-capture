    ...
    public void Scan_And_Capture(String id) throws Exception {
        ...
        try {
...

            {
                ...
                logStep("Opening invoice URL: " + finalUrl);
                ...
                if (savedPath != null) {
                    ...
                    if (!latestPdf.exists()) {
                        ...
                        try {
                            ...
                            logStep("PDF text extracted (" + (Is_OCR ? "via OCR" : "text layer") + ") for id: " + id);
...
//
            ...
            // Mark this id's report node red, then re-throw so TestNG fails
            // only this invocation (other ids continue).
            if (ExtentTestListener.getTest() != null) {
                ExtentTestListener.getTest().fail("Error while processing id " + id + ": " + e.getMessage());
            }
            throw e;
        }
    }
